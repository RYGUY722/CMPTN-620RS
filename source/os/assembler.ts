module TSOS {
	export class Assembler {
		constructor(public Assembly: string = "", 
					private vars: string[] = [],
					private code: string[] = []
					) {
		}
		...
		private constTest(test: String): boolean { // Returns true if the given string is a constant value, false if it's a target address.
			if(test.charAt(0) == '#') {
				return true;
			}
			return false;
